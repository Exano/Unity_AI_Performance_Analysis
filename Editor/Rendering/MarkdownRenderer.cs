    ...
    public static class MarkdownRenderer
    {
        ...
        static List<Block> ParseBlocks(string markdown)
        {
            ...
            while (i < lines.Length)
            {
                ...
                { blocks.Add(new Block { Type = BlockType.HorizontalRule }); i++; continue; }
        ...
        static VisualElement RenderBlock(Block block)
        {
            switch (block.Type)
            {
                ...
                case BlockType.UnorderedList: return RenderList(block, false);
                case BlockType.OrderedList: return RenderList(block, true);
        ...
        static string InlineFormat(string text)
        {
            ...
            { codeSpans.Add(m.Groups[1].Value); return $"\x00CODE{codeSpans.Count - 1}\x00"; });
            text = Regex.Replace(text, @"\*\*(.+?)\*\*", "<b>$1</b>");
            text = Regex.Replace(text, @"(?<!\*)\*(?!\*)(.+?)(?<!\*)\*(?!\*)", "<i>$1</i>");
            text = Regex.Replace(text, @"\[([^\]]+)\]\([^\)]+\)", "<u>$1</u>");
            for (int i = 0; i < codeSpans.Count; i++)
                text = text.Replace($"\x00CODE{i}\x00", $"<color=#6cacee>{codeSpans[i]}</color>");
            return text;
        }
    }
}
